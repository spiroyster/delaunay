...
namespace
{
	...
	std::vector<delaunay::triangle> readOBJ(const std::string& filename)
	{
		...
		std::for_each(obj->faces_.begin(), obj->faces_.end(), 
			...
			{
				result.push_back(delaunay::triangle{ static_cast<unsigned int>(f.vertices_[0].p_ - 1), static_cast<unsigned int>(f.vertices_[1].p_ - 1), static_cast<unsigned int>(f.vertices_[2].p_ - 1) });
			});
...
{
	check("triangle.csv", "triangle.obj");
}
TEST_CASE("Tessellate square", "[delaunay_tessellate_sqaure]")
{
	check("square.csv", "square.obj");
}
TEST_CASE("Tessellate ellipse", "[delaunay_tessellate_ellipse]")
{
	check("ellipse.csv", "ellipse.obj");
}
